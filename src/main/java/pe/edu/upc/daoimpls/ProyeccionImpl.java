...
public class ProyeccionImpl implements IProyeccionDao {
	...
	public List<Proyeccion> findByNameDetalle(Proyeccion y) {
		...
		try {
			...
			Query q=em.createQuery("from Proyeccion y where y.TPDetalles like ?1");
			q.setParameter(1, "%"+y.getTPDetalles()+"%");
			lista= (List<Proyeccion>) q.getResultList();
			
		} catch (Exception e) {
			
			System.out.println("Error al buscar detalles en el daoimpl");
		}
